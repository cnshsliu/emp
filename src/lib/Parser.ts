...
*/
   ...
   */
  getLeaderByPosition: async function (tenant, uid, rdsPart) {
   ...
   * @return {...} An array of {uid, cn}
   */
  getPeerByPosition: async function (tenant, uid, rdsPart) {
   ...
   * @return {...} An array of {uid, cn}
   */
  getStaffByQuery: async function (tenant, uid, rdsPart) {
   ...
   * @return {...}
   */
  getDoerByTeam: async function (tenant, teamid, rdsPart, starter, wfRoot = null) {
    let ret = [];
    let roles = rdsPart
      .split(":")
      .map((x) => x.trim())
  ...
  removeOneUserToRoleResolver: async (tenant, arr, user) => {
    try {
      if (!user) return;
      let uid = null;
      //找到用户的UID
  ...
  addOneUserToRoleResolver: async (tenant, arr, user) => {
    try {
      if (!user) return;
      let uid = null;
      //找到用户的UID
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (Array.isArray(tmp)) {
        for (let i = 0; i < tmp.length; i++) {
          if (isWhiteList) ret = await Parser.addOneUserToRoleResolver(tenant, ret, tmp[i]);
          else ret = await Parser.removeOneUserToRoleResolver(tenant, ret, tmp[i]);
        }
      } else {
        if (typeof tmp === "string") {
          ...
          if (isWhiteList) ret = await Parser.addOneUserToRoleResolver(tenant, ret, tmp);
          else ret = await Parser.removeOneUserToRoleResolver(tenant, ret, tmp);
        }
      }
    }
    return ret;
