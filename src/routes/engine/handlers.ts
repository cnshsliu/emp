...
import { Snapshot, SnapshotType } from "../../database/models/Snapshot";
...
const CheckKsAdminPermission = async (cred: any) => {
  if (cred.employee.group !== "ADMIN" || cred.tenant.domain !== (await Cache.getKsAdminDomain()))
    throw new EmpError(
      ...
      `Only ks admin domain [${await Cache.getKsAdminDomain()}] administrators are allowed for this operation, yours: ${cred.tenant.domain
      }`,
    );
};
...
*/
  ...
  KsTplUploadOne: async (req: Request, h: ResponseToolkit) => {
    return replyHelper.buildResponse(
      h,
      await MongoSession.noTransaction(async () => {
        const PLD = req.payload as any;
