	...
	const askCaishen = async (): Promise<boolean> => {
		...
		if ($page.data.user) {
			...
			if (resultText) {
				...
				const ret = await fetch('http://localhost:6008/caishen/wxSearch', {
					method: 'POST',
	...
	const checkSearchResult = async () => {
		...
		if (intervalId_checkSearchResult) {
			try {
				...
				await fetch('http://localhost:6008/caishen/stopSearcher', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
		...
		intervalId_checkSearchResult = setInterval(async () => {
			...
			console.log(theBody);
			const ret = await fetch('http://localhost:6008/caishen/getOneResult', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: theBody,
			});
